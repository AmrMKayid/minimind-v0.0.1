...
function _tpu_ips {
    ...
    tpu_project=$2
    tpu_name=$3
    gcloud alpha compute tpus tpu-vm describe $tpu_name --zone $tpu_zone --project $tpu_project | grep 'ipAddress' | awk '{print $2}'
    # gcloud alpha compute tpus tpu-vm describe $tpu_name --zone $tpu_zone --project $tpu_project | grep -oP 'ipAddress: \K(.+)$'
}
...
function tpu {
    ...
    export PROJECT_HOME='..'
    export PROJECT_NAME='minimind'
    tpu_zone='us-central2-b'
    if [ "$1" = "minimind" ]; then
        tpu_project='valued-sight-253418'
        tpu_zone='us-central2-b'
        tpu_gen='v4'
    else
        echo "Invalid syntax!"
...
export tpu _tpu_ips _tpu_create _tpu_setup _tpu_check _tpu_copy _tpu_stop _tpu_launch _tpu_maintain _tpu_ssh _tpu_reboot
